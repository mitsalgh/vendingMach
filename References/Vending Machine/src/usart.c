...
uint8_t send_byte(uint8_t usart_number, uint8_t b) {
    return write_buffer(usart_number, TX, b);
}

uint8_t recv_byte(uint8_t usart_number) {
    uint8_t b;
    read_buffer(usart_number, RX, &b);
    return b;
}
